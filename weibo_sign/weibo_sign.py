...
if __name__ == "__main__":
    succeeded = False
    # 获取参数
    #获取微博超话链接，获取方法 打开手机轻享版-》关注的超话，复制链接填入weibo_my_cookie参数中
    weibo_my_cookie = ''

    params = extract_params(weibo_my_cookie)


    #params=weibo_my_cookie
    message_to_push = ""
    while not succeeded:
        try:
            ...
            while True:
                ...
                # 检查是否存在 sign_action 需要签到的超话
                has_sign_action = any(
                    topic.get("sign_action") != "" for topic in topics
                )
                # 如果没有 sign_action 需要签到的超话，则退出外部循环
